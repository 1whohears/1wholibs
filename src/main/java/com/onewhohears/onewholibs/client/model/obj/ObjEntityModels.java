...
public class ObjEntityModels implements ResourceManagerReloadListener {
	...
	public static class ModelOverrides {
		...
		public void apply(PoseStack poseStack) {
			if (isNone()) return;
			if (rotation[0] != 0) poseStack.mulPose(VectorUtils.rotationQuaternion(VectorUtils.POSITIVE_X, rotation[0]));
			if (rotation[1] != 0) poseStack.mulPose(VectorUtils.rotationQuaternion(VectorUtils.NEGATIVE_Y, rotation[1]));
			if (rotation[2] != 0) poseStack.mulPose(VectorUtils.rotationQuaternion(VectorUtils.POSITIVE_Z, rotation[2]));
