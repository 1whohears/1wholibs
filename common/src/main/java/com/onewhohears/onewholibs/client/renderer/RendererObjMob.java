...
public class RendererObjMob<T extends Mob> extends RendererObjLiving<T> {

    public RendererObjMob(EntityRendererProvider.Context ctx, ObjEntityModel<T> model) {
        super(ctx, model);
    }
    ...
    private <E extends Entity> void renderLeash(T mob, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, E entity) {
        ...
        double d = (double)(Mth.lerp(f, mob.yBodyRotO, mob.yBodyRot) * ((float)Math.PI / 180F)) + (Math.PI / 2D);
